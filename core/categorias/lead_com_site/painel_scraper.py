...
class _MetricasLead:
    ...
    # ``sessoes_semana`` permanece para uso interno mas não é exposto via placeholder.
    ...
    def as_placeholders(self, sufixo: str = "") -> Dict[str, str]:  # noqa: D401
        """Gera o dicionário de placeholders para o Google Slides.

        Obs.: O placeholder de *sessões* foi removido, pois o número de
        sessões será tratado em outra parte do pipeline.
        """
        return {
...
def coletar_metricas_leads(  # noqa: D401
    ...
    O número de sessões (*sessoes_semana*) deve ser fornecido pelo caller –
    ele não é mais extraído da planilha.
    """
    ...
    # 3) Validação de colunas (SESSÕES removido)
    ...
    # 5) Conversão numérica padrão BRL/pt‑BR (SESSÕES removido)
    numeric_cols = [
        "VALOR INVESTIDO",
        "LEADS",
        "META INVESTIMENTO",
    ...
    # ------------------------------------------------------------------
    # Sessões: valor fornecido externamente
    # ------------------------------------------------------------------
    sessoes_semana_val = sessoes_semana

    cpl_semana = (
        None if not inv_semana or not lead_semana else inv_semana / lead_semana
    )
    cps_semana = (
        None if not inv_semana or not sessoes_semana_val else inv_semana / sessoes_semana_val
    )
    lps_semana = (
        None if not lead_semana or not sessoes_semana_val else lead_semana / sessoes_semana_val
    )
